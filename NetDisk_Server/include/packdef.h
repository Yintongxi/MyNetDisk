...
struct STRU_FILE_Info
{
    ...
    char uploadTime[_MAX_SIZE];
    int size;//约定  小型网盘 不上传2G以上的文件
    //int型 2 147 483 647  2GB
    char dir[_MAX_PATH_SIZE];
    char md5[_MAX_SIZE];//32位16进制数
...
{
    STRU_GET_UPLOAD_POS_RQ():type(_DEF_PACK_GET_UPLOAD_POS_RQ){
        userid = 0;
        fileid = 0;
        memset(dir , 0 ,sizeof(dir));
    }
...
////////////////下载续传协议////////////////
//请求
//告诉服务器 从哪里开始传数据块就可以直接传了，不需要回复
#define _DEF_PACK_CONTINUE_DOWNLOAD_RQ (_DEF_PROTOCOL_BASE + 27 )

//服务器中 map 有和没有   有 fileid 没有需要唯一确认文件 uid fid fdir
...
/// 服务器 map 有 userid + fileid 没有 查表 与下载续传查表 流程一样
//请求
...
struct STRU_CONTINUE_UPLOAD_RS{
    ...
    int fileid;
    int pos;
    //char dir[_MAX_PATH_SIZE]; 上传不可以有相同fileid , 所以fileid 就可以唯一确认了
